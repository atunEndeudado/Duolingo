...
class UsuarioService:
    ...
    def activar_premium(self, usuario_id: int) -> UsuarioResponseDTO:
        usuario = self.repository.obtener_por_id(usuario_id)
        if not usuario:
            raise ValueError("Usuario no encontrado")
        
        usuario_actualizado = self.repository.activar_premium(usuario)
        return to_usuario_response(usuario_actualizado)
